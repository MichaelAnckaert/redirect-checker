...
def check_redirects(from_url, to_url):
    ...
    if response.status_code == 404:
        ...
        if to_url != response.next.url:
            print(f"ERROR: Wrong 301 redirect for {from_url}! Expected '{to_url}' but got '{response.next_url}'")
        else:
...
def get_redirects(filename: str, skip_header: bool, from_domain: Optional[str], to_domain: Optional[str]):
    redirects = []
